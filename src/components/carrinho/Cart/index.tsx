...
export const Cart = () => {
    ...
    const formik = useFormik({
        ...
        validationSchema: yup.object({
            receiver: yup.string().min(3, 'Por favor, digite um nome válido').required('Por favor, digite um nome'),
            adressDescription: yup.string().min(5, 'Por favor, digite um endereço válido').required('Este campo é obrigatório'),
            city: yup.string().min(4, 'Por favor, digite uma cidade válida').required('Este campo é obrigatório'),
            zipCode: yup.number().min(8, 'Digite um CEP que exista').required('Este campo é obrigatório'),
            nameCard: yup.string().min(3, 'Por favor, digite um nome válido').required('Este campo é obrigatório'),
            cardNumber: yup.number().required('Este campo é obrigatório'),
            cardCode: yup.number().max(999, 'Máximo permitido: 999').min(1, 'Valor mínimo: 1').required('Este campo é obrigatório'),
            expirationMonth: yup.number().min(1, 'Digite um mês válido').max(12, 'Digite um mês válido').required('Este campo é obrigatório'),
            expirationYear: yup.number().min(2024, 'Digite um ano válido').max(9999, 'Digite um ano válido').required('Este campo é obrigatório'),
        }),
        onSubmit: (values) => {
            purchase({
                products: [{
                            ...
                            <>
                                ...
                                {items.length > 0 ? (
                                    <>
                                        ...
                                        {!isTyping && (<stl.Button onClick={() => setIsTyping(true)}>Continuar com a compra</stl.Button>)}
                                    </>
                                ) :
                                <stl.CarrinhoVazio>
                                    Nada por aqui... <br />
                                    ...
                                    <>
                                        <stl.Form onSubmit={formik.handleSubmit}>
                                            ...
                                            <stl.InputContainer>
                                                ...
                                                />
                                                <small>{getFormError('receiver', formik.errors.receiver)}</small>
                                            </stl.InputContainer>
                                            <stl.InputContainer>
                                                ...
                                                <stl.Input
                                                    ...
                                                    onBlur={formik.handleBlur}
                                                />
                                                <small>{getFormError('adressDescription', formik.errors.adressDescription)}</small>
                                            </stl.InputContainer>
                                            <stl.InputContainer>
                                            ...
                                            <stl.InputContainer display="flex">
                                                <stl.InputGroup width="60" display="flex">
                                                    ...
                                                    <stl.Input
                                                        ...
                                                        onBlur={formik.handleBlur}
                                                    />
                                                    <small>{getFormError('zipCode', formik.errors.zipCode)}</small>  
                                                </stl.InputGroup>
                                                <stl.InputGroup width="30" display="flex">
                                                    ...
                                                    <stl.Input
                                                        ...
                                                        onBlur={formik.handleBlur}
                                                    />
                                                    <small>{getFormError('number', formik.errors.number)}</small>
                                                </stl.InputGroup>
                                            </stl.InputContainer>
                                            <stl.InputContainer>
                                                ...
                                                <stl.Input
                                                    ...
                                                    onBlur={formik.handleBlur}
                                                />
                                                <small>{getFormError('complement', formik.errors.complement)}</small>
                                            </stl.InputContainer>
                                            <stl.Button type="button" onClick={() => setIsPaying(true)}>Continuar para pagamento</stl.Button>
                                            <stl.Button onClick={() => setIsTyping(false)}>Voltar para produtos</stl.Button>
                                        </stl.Form>
                                    </>
                                )}
                                        ...
                                        <stl.Form onSubmit={formik.handleSubmit}>
                                            ...
                                            <stl.InputContainer>
                                                ...
                                                <stl.Input
                                                    ...
                                                    onBlur={formik.handleBlur}
                                                />
                                                <small>{getFormError('nameCard', formik.errors.nameCard)}</small>
                                            </stl.InputContainer>
                                            <stl.InputContainer display="flex">
                                                <stl.InputGroup display="flex" width="60">
                                                    ...
                                                    <stl.Input
                                                        ...
                                                        onBlur={formik.handleBlur}
                                                    />
                                                    <small>{getFormError('cardNumber', formik.errors.cardNumber)}</small>
                                                </stl.InputGroup>
                                                <stl.InputGroup width="30" display="flex">
                                                    ...
                                                    <stl.Input
                                                        ...
                                                        onBlur={formik.handleBlur}
                                                    />
                                                    <small>{getFormError('cardCode', formik.errors.cardCode)}</small>
                                                </stl.InputGroup>
                                            </stl.InputContainer>
                                            <stl.InputContainer display="flex">
                                                <stl.InputGroup width="45" display="flex">
                                                    ...
                                                    <stl.Input
                                                        ...
                                                        onBlur={formik.handleBlur}
                                                    />
                                                    <small>{getFormError('expirationMonth', formik.errors.expirationMonth)}</small>
                                                </stl.InputGroup>
                                                <stl.InputGroup width="45" display="flex">
                                                    ...
                                                    <stl.Input
                                                        ...
                                                        onBlur={formik.handleBlur}
                                                    />
                                                    <small>{getFormError('expirationYear', formik.errors.expirationYear)}</small>
                                                </stl.InputGroup>
                                            </stl.InputContainer>
                                            <stl.Button type="submit">Finalizar compra</stl.Button>
                                            <stl.Button onClick={() => setIsPaying(false)}>Voltar para entrega</stl.Button>
                                        </stl.Form>
                                    </>
                                )}
                            </>
